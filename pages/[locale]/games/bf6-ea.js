...
export default function BF6EventData() {
  ...
  useEffect(() => {
    ...
    if (
      weekParam &&
      (weekParam === '1' || weekParam === '2' || weekParam === 'all')
    ) {
      const weekValue =
        weekParam === '1'
          ? 'OpenBetaWeekend1'
    ...
    try {
      ...
      if (week === 'all') {
        ...
        const [week1Response, week2Response] = await Promise.all([
          axios.get(
            `/api/bf6-stats?locale=${currentLocale}&user=${user}&week=OpenBetaWeekend1`
          ),
          axios.get(
            `/api/bf6-stats?locale=${currentLocale}&user=${user}&week=OpenBetaWeekend2`
          ),
        ]);
        ...
        const response = await axios.get(
          `/api/bf6-stats?locale=${currentLocale}&user=${user}&week=${week}`
        );
  ...
  const combineWeekData = (week1Data, week2Data) => {
    ...
    const combinedStats = {
      total_kills: {
        ...
        value: (
          parseInt(week1Stats.total_kills.value.replace(',', '')) +
          parseInt(week2Stats.total_kills.value.replace(',', ''))
        ).toString(),
      },
      total_assists: {
        ...week1Stats.total_assists,
        value: (
          parseInt(week1Stats.total_assists.value.replace(',', '')) +
          parseInt(week2Stats.total_assists.value.replace(',', ''))
        ).toString(),
      },
      total_revives: {
        ...week1Stats.total_revives,
        value: (
          parseInt(week1Stats.total_revives.value.replace(',', '')) +
          parseInt(week2Stats.total_revives.value.replace(',', ''))
        ).toString(),
      },
      total_matches_played: {
        ...week1Stats.total_matches_played,
        value: (
          parseInt(week1Stats.total_matches_played.value.replace(',', '')) +
          parseInt(week2Stats.total_matches_played.value.replace(',', ''))
        ).toString(),
      },
      total_xp: {
        ...week1Stats.total_xp,
        value: (
          parseInt(week1Stats.total_xp.value.replace(/,/g, '')) +
          parseInt(week2Stats.total_xp.value.replace(/,/g, ''))
        ).toLocaleString(),
      },
      kill_death_ratio:
        week1Stats.kill_death_ratio || week2Stats.kill_death_ratio,
    ...
    const week1Matches = parseInt(
      week1Stats.total_matches_played.value.replace(',', '')
    );
    const week2Matches = parseInt(
      week2Stats.total_matches_played.value.replace(',', '')
    );

    const mostPlayedClass =
      week1Matches >= week2Matches
        ? week1Data.mostPlayedClass
        : week2Data.mostPlayedClass;
    const mostPlayedMode =
      week1Matches >= week2Matches
        ? week1Data.mostPlayedMode
        : week2Data.mostPlayedMode;
  ...
  const handleWeekChange = async (week) => {
    ...
    if (week === 'all') {
      ...
      try {
        const [week1Response, week2Response] = await Promise.all([
          axios.get(
            `/api/bf6-stats?locale=${locale}&user=${currentUser}&week=OpenBetaWeekend1`
          ),
          axios.get(
            `/api/bf6-stats?locale=${locale}&user=${currentUser}&week=OpenBetaWeekend2`
          ),
        ]);
      ...
      try {
        const response = await axios.get(
          `/api/bf6-stats?locale=${locale}&user=${currentUser}&week=${week}`
        );
  ...
  const handleShare = () => {
    ...
    const weekText =
      selectedWeek === 'all' ? 'All Weeks' : `Weekend ${weekParam}`;
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Battlefield 6 Open Beta Weekend 2 player statistics and achievements"
        />
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-left">
                        <h2 className="text-2xl font-bold text-white font-mono">
                          {data.playerDisplayName}
                        </h2>
                        <p className="text-gray-400 font-mono text-sm">
                          SOLDIER: {data.playerDisplayName}
                        </p>
                      </div>
                    </div>
          ...
          <div className="container mx-auto px-4">
            ...
            >
              ...
              >
                ...
                <h4 className="text-xl font-bold text-orange-400 mb-1 font-mono">
                  {mostPlayedClass.name}
                </h4>
                <p className="text-gray-300 text-xs leading-relaxed mb-2 flex-grow">
                  {mostPlayedClass.description}
                </p>
                <img
              ...
              >
                <div className="text-center">
                  ...
                  <div className="text-5xl font-bold text-orange-400 mb-1 font-mono">
                    {stats.total_matches_played.value}
                  </div>
                  <p className="text-gray-300 font-mono text-sm">
                    {stats.total_matches_played.name}
                  </p>
                </div>
              </motion.div>
            </motion.div>
            ...
            >
              ...
              >
                ...
                />
                <div className="text-2xl font-bold text-orange-400 mb-1 font-mono">
                  {stats.total_revives.value}
                </div>
                <p className="text-gray-300 text-xs font-mono">
                  {stats.total_revives.name}
                </p>
              </motion.div>
            ...
            >
              ...
              >
                <h3 className="text-lg font-bold text-white mb-2 text-center font-mono">
                  K/D RATIO
                </h3>
                <div className="text-center">
                  <div className="text-3xl font-bold text-orange-400 mb-1 font-mono">
                    {stats.kill_death_ratio
                      ? parseFloat(stats.kill_death_ratio).toFixed(2)
                      : 'N/A'}
                  </div>
                  <p className="text-gray-300 text-xs font-mono">
                    KILLS PER DEATH
              ...
              >
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-orange-400 mb-1 font-mono">
                    {(
                      parseInt(stats.total_kills.value) /
                      parseInt(stats.total_matches_played.value)
                    ).toFixed(1)}
                  </div>
              ...
              >
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-orange-400 mb-1 font-mono">
                    {(
                      parseInt(stats.total_xp.value.replace(',', '')) /
                      parseInt(stats.total_matches_played.value)
                    ).toLocaleString()}
                  </div>
                  <p className="text-gray-300 text-xs font-mono">PER MISSION</p>
            ...
            >
              <p className="text-gray-400 text-sm font-mono mb-4">
                COMBAT DATA FROM EA BATTLEFIELD 6 OPEN BETA{' '}
                {selectedWeek === 'OpenBetaWeekend1'
                  ? 'WEEKEND 1'
                  : selectedWeek === 'OpenBetaWeekend2'
                    ? 'WEEKEND 2'
              ...
              <div className="flex space-x-3 justify-center">
                <button
                  onClick={() =>
                    fetchBF6Data(locale, currentUser, selectedWeek)
                  }
