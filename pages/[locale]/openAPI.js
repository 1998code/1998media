...
export default function openAPI(props) {
  function i18n(key) {
    ...
    return props.i18n && props.i18n['openAPI'] && props.i18n['openAPI'][key] ? props.i18n['openAPI'][key] : key
  }
  ...
  return (
    <div id="openAPI" data-aos="zoom-in" data-aos-once className="relative pt-16 md:py-20 px-4 sm:px-6 lg:px-8">
      <div className="relative max-w-7xl mx-auto">
        <div className="text-left flex flex-wrap">
          <a className="text-3xl tracking-tight font-extrabold text-gray-900 dark:text-gray-100 sm:text-4xl grow" href="#openAPI">
            {i18n("OpenAPI")}
        ...
        <div className="bg-white dark:bg-black shadow overflow-hidden rounded-2xl mt-8 backlight">
          <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-800">
            {lists.map((item) => (
              <li key={item.title} onClick={() => setHighlighted(item.title)} className="cursor-pointer">
                <div className={`block ${item.bgColor} opacity-90 hover:opacity-100`}>
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex gap-3">
                        <span className={`w-32 border px-2 inline-flex items-center text-xs leading-5 font-semibold rounded-full ${item.textColor} ${item.bgColor}`}>
                            {i18n(item.type)}
                        </span>
                        <div className={classNames(item.textColor, 'font-medium truncate')}>
                          {i18n(item.title)}
                        </div>
                      </div>
                    ...
                    <div className={`${highlighted === item.title ? 'flex' : 'hidden'} mt-2 flex-wrap justify-between gap-3 text-xs`}>
                      <code className={`flex-1 ${item.textColor} bg-white dark:bg-black/50 p-2 rounded-md`}>
                        {i18n(item.code)}
                      </code>
