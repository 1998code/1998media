        ...
        <div className="mt-8">
            ...
            { dalle.map((item, index) => (
              item.featured === '🏆 Hall of Fame' && (
                <a href={item.sourceURL} target="_blank" rel="noopener noreferrer" className="group w-96">
                  <img src={item.output} className="min-w-[350px] min-h-[350px] rounded-lg shadow-lg group-hover:scale-95 border border-transparent hover:border-black dark:hover:border-white backlight transform transition duration-500" loading="lazy" />
          ...
          <div className="flex flex-nowrap overflow-x-auto max-w-full gap-6 mt-8">
            { dalle.map((item, index) => (
              item.featured !== '🏆 Hall of Fame' && (
                <a href={item.sourceURL} target="_blank" rel="noopener noreferrer" className="group w-96">
                  <img src={item.output} className="min-w-[280px] min-h-[280px] rounded-lg shadow-lg group-hover:scale-95 transition-all" loading="lazy" />
                  <div className="mt-2 flex justify-between">
