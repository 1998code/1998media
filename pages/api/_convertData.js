export default function (req, res) {
    ...
    // Convert to CSV
    const csv = convertToCSV(i18nRaw)

    res.status(200).send(csv);
}

function convertToCSV(i18nRaw) {
    const headers = ['position', 'key', 'en', 'zh', 'zh-HK', 'ko', 'ja'];
    let rows = [];
    let position = 1;
    for (let category in i18nRaw['en']) {
      for (let key in i18nRaw['en'][category]) {
        const row = [
          category,
          `"${key}"`,
          `"${i18nRaw['en'][category][key] || key}"`,
          `"${i18nRaw['zh'][category][key] || key}"`,
          `"${i18nRaw['zh-HK'][category][key] || key}"`,
          `"${i18nRaw['ko'][category][key] || key}"`,
          `"${i18nRaw['ja'][category][key] || key}"`
        ];
        rows.push(row.join(','));
        position++;
      }
