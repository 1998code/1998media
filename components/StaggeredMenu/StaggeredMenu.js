...
export const StaggeredMenu = ({
    ...
    const buildOpenTimeline = useCallback(() => {
        ...
        if (topButtonEl) {
            tl.to(topButtonEl, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, panelInsertTime + 0.2);
        }
        ...
        if (itemEls.length) {
            ...
            if (iconEls.length) {
                tl.to(iconEls, { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out', stagger: { each: 0.08, from: 'start' } }, itemsStart + 0.1);
            }
            if (numberEls.length) {
                tl.to(numberEls, { duration: 0.6, ease: 'power2.out', '--sm-num-opacity': 1, stagger: { each: 0.06, from: 'start' } }, itemsStart + 0.15);
            }
    ...
    const animateIcon = useCallback(opening => {
        ...
        if (opening) {
            ...
            spinTweenRef.current = gsap.timeline({ defaults: { ease: 'power4.out' } })
                .to(h, { rotate: 45, duration: 0.5 }, 0).to(v, { rotate: -45, duration: 0.5 }, 0);
        } else {
    ...
    const toggleMenu = useCallback(() => {
        ...
        animateText(target);
        animateColor(target);
    }, [onMenuOpen, onMenuClose, playOpen, playClose, animateIcon, animateText, animateColor]);
    ...
    const Overlay = (
        <div className={`sm-scope sm-portal-root fixed inset-0 z-[9997] transition-all ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`} style={{ '--sm-accent': accentColor }} onClick={(e) => {
            if (panelRef.current && !panelRef.current.contains(e.target)) {
                closeMenu();
            }
        }}>
            <div ref={preLayersRef} className="sm-prelayers fixed inset-0 pointer-events-none z-[9998]">
                {(colors || []).slice(0, 3).map((c, i) => (
                    <div key={i} className="sm-prelayer absolute inset-0 translate-x-0" style={{ background: c }} />
                ))}
            </div>
            <aside ref={panelRef} className="staggered-menu-panel fixed top-0 right-0 h-full bg-white/90 backdrop-blur-xl flex flex-col p-[3em_2em_2em_2em] overflow-y-auto z-[9999] pointer-events-auto" data-position={position}>
                <div className="sm-panel-inner flex-1 flex flex-col">
                    {topButton && (
                        <div className="sm-panel-top mb-6 cursor-pointer" onClick={closeMenu}>
                            {topButton}
                        </div>
                    )}
                    <ul className="sm-panel-list list-none m-0 p-0 flex flex-col gap-2" data-numbering={displayItemNumbering}>
                        {items.map((it, idx) => (
                            <li key={idx} className="sm-panel-itemWrap overflow-hidden">
                                <a className="sm-panel-item group/item block text-[1.8rem] font-bold uppercase tracking-tighter hover:text-[var(--sm-accent)] transition-all no-underline flex items-center gap-4 py-1" href={it.link} onClick={(e) => {
                                    ...
                                    {it.icon && <span className="sm-panel-itemIcon inline-block text-[1em] transition-all duration-300 group-hover/item:text-[var(--sm-accent)] group-hover/item:translate-x-2">{it.icon}</span>}
                                    <span className="sm-panel-itemLabel inline-block will-change-transform transform-origin-bottom group-hover/item:translate-x-2 transition-transform duration-300">{it.label}</span>
                                </a>
                            </li>
                        ))}
                    </ul>
                    {displaySocials && socialItems.length > 0 && (
                        <div className="sm-socials mt-auto pt-10">
                            <div className="flex flex-row items-center gap-6 sm-socials-list">
                                {socialItems.map((s, i) => (
                                    <a key={i} href={s.link} target="_blank" rel="noreferrer" className="sm-socials-link transition-colors" title={s.ariaLabel || s.label} style={{ color: 'inherit' }}>
    ...
    return (
        <div className={`sm-scope-root ${className || ''}`} style={{ '--sm-accent': accentColor }}>
            <button ref={toggleBtnRef} className={`sm-toggle flex items-center gap-2 bg-transparent border-0 cursor-pointer ${buttonClassName || ''}`} onClick={toggleMenu}>
                {triggerIcon ? (
                    <span className="transition-transform duration-300" style={{ transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>{triggerIcon}</span>
                ) : (
                    <>
                        <div className="sm-toggle-textWrap h-[1em] overflow-hidden">
                            <div ref={textInnerRef} className="flex flex-col">
                                {textLines.map((l, i) => <span key={i} className="leading-none">{l}</span>)}
                            </div>
                        </div>
                        <div ref={iconRef} className="relative w-4 h-4 flex items-center justify-center">
            ...
            {mounted && (forceOverlay ? createPortal(Overlay, document.body) : Overlay)}
