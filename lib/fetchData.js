...
export async function fetchI18nData(locale) {
  try {
    ...
    // If language is supported, read from local JSON file
    if (availableLangs.includes(normalizedLocale)) {
      try {
        ...
        console.error(`Error reading i18n file for ${normalizedLocale}:`, err);
        // Fallback to English if file read fails
      }
    }

    // Language not supported or file read failed - use English as base
    let englishData = {};
    try {
      const filePath = path.join(process.cwd(), 'data', 'i18n', 'en.json');
      const fileContent = fs.readFileSync(filePath, 'utf8');
      englishData = JSON.parse(fileContent);
    } catch (err) {
      console.error('Error reading English i18n file:', err);
      return {};
    }
