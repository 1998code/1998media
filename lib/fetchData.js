...
export async function fetchTripMedals(locale) {
  try {
    ...
    const tripLocale = locale === 'ja' ? 'ja-JP' : 
                       locale === 'ko' ? 'ko-KR' :
                       locale === 'zh' ? 'zh-CN' :
                       locale === 'zh-HK' ? 'zh-TW' :
                       locale === 'en' ? 'en-US' :
    ...
    if (!response.ok) {
      console.warn('Trip.com medals API returned non-OK status:', response.status);
      return [];
    }
...
export async function fetchTripMoments(locale) {
  try {
    ...
    // Check if response is OK and has JSON content
    if (!response.ok) {
      console.warn('Trip.com moments API returned non-OK status:', response.status);
      return [];
    }
  ...
  try {
    ...
    const promises = symbolList.map(async (symbol) => {
      try {
        ...
        const currentPrice = meta.regularMarketPrice || quote.close?.[latestIndex] || meta.chartPreviousClose || 0;
        ...
        const marketOpen = tradingPeriod.start && tradingPeriod.end && 
                          now >= tradingPeriod.start && now <= tradingPeriod.end;
        const marketState = marketOpen ? 'REGULAR' : 'CLOSED';

        // Extract chart data (closing prices and timestamps)
        const chartData = quote.close?.filter((price, idx) => price != null && timestamps[idx] != null) || [];
        const chartTimestamps = timestamps.filter((ts, idx) => quote.close?.[idx] != null) || [];
