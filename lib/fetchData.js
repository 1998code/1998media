...
// Portfolio holdings and stock fetching live in a dependency-free module so
// they can be shared with the edge API route without bloating its bundle.
export { MY_PORTFOLIO, fetchStocks } from './portfolio';
...
export async function fetchI18nData(locale) {
  try {
    const availableLangs = ['en', 'zh', 'zh-HK', 'ko', 'ja', 'ru', 'fr', 'es'];
...
// Medium RSS feeds. English/global posts come from the two English
// publications; Chinese posts from the Chinese publication. The blog page
// routes posts to the right locale by detecting each title's language, so all
// feeds are merged here and split for display downstream.
const BLOG_FEED_URLS = [
  'https://medium.com/feed/fly-trip',
  'https://medium.com/feed/tech-blog-today',
  'https://medium.com/feed/文章中文',
];

/**
 * Parse Medium RSS XML into blog post items.
 */
function parseBlogFeed(xml) {
  ...
  for (const itemXml of itemMatches) {
    ...
    // Medium puts the image inside the description or content:encoded HTML —
    // extract the first <img src> from whichever one has it.
    const html = `${getTag('description')} ${getTag('content:encoded')}`;
...
/**
 * Fetch blog posts from the Medium RSS feeds (parse XML directly), merged and
 * sorted newest-first.
 */
export async function fetchBlogPosts() {
  try {
    const results = await Promise.allSettled(
      BLOG_FEED_URLS.map(async (feedUrl) => {
...
export async function fetchTripMoments(locale) {
  try {
    // Normalize locale for Trip.com API (requires full locale codes like ja-JP, not just ja)
    // Fallback to English if language not found.
    // Trip.com returns no moments for zh-CN, so Simplified ('zh') uses the
    // zh-TW feed and the titles are converted to Simplified on the client.
    const tripLocale =
      locale === 'ja'
        ? 'ja-JP'
        : locale === 'ko'
